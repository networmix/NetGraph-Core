...
def test_ksp_line1_two_paths(line1_graph, t_cost, assert_pred_dag_integrity):
    g = line1_graph
...
def test_ksp_square1_two_paths(square1_graph, t_cost, assert_pred_dag_integrity):
    g = square1_graph
    items = ngc.ksp(g, 0, 2, k=5, max_cost_factor=None, unique=True)
    assert len(items) >= 2
    costs = [t_cost(dist, 2) for dist, _ in items[:2]]
...
def test_ksp_fully_connected_costs(
    make_fully_connected_graph, t_cost, assert_pred_dag_integrity
):
...
def test_ksp_graph5_thresholds(graph5, t_cost, assert_pred_dag_integrity):
    g = graph5
...
def test_ksp_square5_routes(square5_graph, t_cost, assert_pred_dag_integrity):
    g = square5_graph
